...
export default async function AdminCustomersPage() {
  ...
  // Fetch all bookings to match against users by email
  ...
  const customers = users.map((user) => {
    const userBookings = allBookings.filter(b => b.email && b.email.toLowerCase() === user.email?.toLowerCase());
    ...
    return {
      ...
      totalSpent: userBookings.reduce(
        (sum, booking) => sum + Number(booking.fullPrice),
        0
      ),
    };
  });
