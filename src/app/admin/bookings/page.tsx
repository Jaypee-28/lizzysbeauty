...
import { Booking, Service, BookingStatus } from "@/generated/prisma";
...
export default function AdminBookingsPage() {
  ...
  const getStatusBadge = (status: BookingStatus) => {
    switch (status) {
      ...
      case "PENDING_PAYMENT": return { label: "Pending Payment", className: "bg-purple-50 text-purple-600 border-purple-100", icon: AlertCircle };
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50/50">
                  ...
                  <th className="px-6 py-5 text-left text-xs font-black text-gray-400 uppercase tracking-widest">Financials</th>
                      ...
                      <td className="px-6 py-6">
                        <div className="space-y-1">
                          <div className="flex justify-between gap-4 text-[11px]">
                            ...
                            <span className="font-black text-gray-900">${Number(booking.fullPrice).toFixed(2)}</span>
                          </div>
                          <div className="flex justify-between gap-4 text-[11px]">
                            <span className="font-bold text-gray-500 uppercase">Deposit:</span>
                            <span className="font-black text-emerald-600">${Number(booking.depositPaid).toFixed(2)}</span>
                          </div>
                          <div className="flex justify-between gap-4 text-[11px]">
                            <span className="font-bold text-gray-500 uppercase">Balance:</span>
                            <span className="font-black text-[#FF4D8D]">${Number(booking.outstandingBalance).toFixed(2)}</span>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-6">
              ...
              <div key={booking.id} className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm space-y-5">
                ...
                <div className="grid grid-cols-2 gap-4 py-4 border-y border-gray-50">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs font-bold text-gray-900 flex items-center gap-1 mt-0.5">
                  ...
                  <div className="space-y-1 bg-gray-50 p-2 rounded-xl border border-gray-100">
                    <div className="flex justify-between text-[10px]">
                      ...
                      <span className="font-bold text-gray-900">${Number(booking.fullPrice).toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between text-[10px]">
                      <span className="font-bold text-gray-400">Dep:</span>
                      <span className="font-bold text-emerald-600">${Number(booking.depositPaid).toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between text-[10px] pt-1 mt-1 border-t border-gray-200">
                      <span className="font-bold text-gray-400">Bal:</span>
                      <span className="font-bold text-[#FF4D8D]">${Number(booking.outstandingBalance).toFixed(2)}</span>
                    </div>
                  </div>
                </div>
